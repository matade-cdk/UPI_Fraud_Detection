...
function evaluateRisk(features) {
  ...
  if (finalScore >= 0.8) {
    paymentAdvisory = {
      alert: "HIGH_ALERT",
      paymentAllowed: true,
      message: "High fraud risk detected. You can proceed, but confirm recipient details before payment.",
    };
  } else if (finalScore >= 0.5) {
    paymentAdvisory = {
      alert: "WARNING",
      paymentAllowed: true,
      message: "Risk involved at 50%+ fraud chance. Payment can be done with caution.",
    };
  } else {
    paymentAdvisory = {
      alert: "NONE",
      paymentAllowed: true,
      message: "Low fraud risk. Payment looks safe.",
    };
  }
